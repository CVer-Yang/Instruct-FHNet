...
def CE_Net_Train():
    ...
    NAME = 'Instruct_FHNet' + Constants8.ROOT.split('/')[-1]
    print(NAME)
    # run the Visdom
    # viz = Visualizer(env=NAME)

    solver = MyFrame(Instruct_FHNet, dice_bce_loss, 2e-4)
    #solver.load('./weights/LAVT_context8Masroad_build.th')
    batchsize = torch.cuda.device_count() * Constants8.BATCHSIZE_PER_CARD
    ...
    #############修改自己的数据集地址
    dataset = ImageFolder(root_path=Constants8.ROOT, datasets='Masroadbuild_text', jsoncap_path=Constants8.JSONcap_path,
                          jsoncaplen_path=Constants8.JSONcaplen_path)
    ...
    no_optim = 0
    total_epoch = Constants8.TOTAL_EPOCH
    train_epoch_best_loss = Constants8.INITAL_EPOCH_LOSS
    for epoch in range(1, total_epoch + 1):
        data_loader_iter = iter(data_loader)
        train_epoch_loss = 0
        index = 0

        for img, mask, text, caplen in data_loader_iter:
        ...
        print('SHAPE:', Constants8.Image_size, file=mylog)
        print('********')
        print('epoch:', epoch, '    time:', int(time() - tic))
        print('train_loss:', train_epoch_loss)
        print('SHAPE:', Constants8.Image_size)
        ...
        if no_optim > Constants8.NUM_EARLY_STOP:
            print('early stop at %d epoch' % epoch, file=mylog)
            print('early stop at %d epoch' % epoch)
            break
        if no_optim > Constants8.NUM_UPDATE_LR:
