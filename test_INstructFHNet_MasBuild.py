...
class TTAFrame():
    ...
    def test_one_img_from_path(self, path,text, evalmode=True):
        ...
        print(batchsize)
        if batchsize >= 8:
            return self.test_one_img_from_path_1(path,text)
        elif batchsize >= 4:
            return self.test_one_img_from_path_2(path,text)
        elif batchsize >= 2:
            return self.test_one_img_from_path_4(path,text)

    def test_one_img_from_path_8(self, path, text):
        ...
        maska = self.net.forward(img1,text).squeeze().cpu().data.numpy()
        maskb = self.net.forward(img2,text).squeeze().cpu().data.numpy()
        maskc = self.net.forward(img3,text).squeeze().cpu().data.numpy()
        maskd = self.net.forward(img4,text).squeeze().cpu().data.numpy()
    ...
    def test_one_img_from_path_4(self, path,text):
        ...
        img4 = V(torch.Tensor(np.array(img4, np.float32) / 255.0 * 3.2 - 1.6).cuda())
        n, c, h, w = img1.shape
        text = np.tile(text, (n, 1))
        text = V(torch.tensor(text).cuda())
    ...
    def test_one_img_from_path_2(self, path, text):
        img = cv2.imread(path)  # .transpose(2,0,1)[None]
        img = cv2.resize(img, (1024, 1024))
        img90 = np.array(np.rot90(img))
        img1 = np.concatenate([img[None], img90[None]])
        ...
        maska = self.net.forward(img5,text).squeeze().cpu().data.numpy()  # .squeeze(1)
        maskb = self.net.forward(img6,text).squeeze().cpu().data.numpy()
    ...
    def test_one_img_from_path_1(self, path, text):
        img = cv2.imread(path)  # .transpose(2,0,1)[None]
        # 修改图片尺寸
        img = cv2.resize(img, (1024, 1024))
        ...
        mask = self.net.forward(img5, text).squeeze().cpu().data.numpy()  # .squeeze(1)
...
def test_ce_net_vessel():
    ...
    jsonfile = '/data/yangzhigang/RCFSNet/dataset/Mas/test/test_CAPTIONS_Build_Masroad_Build.json'
    with open(jsonfile, 'r') as j:
        instruction = json.load(j)

    source = './dataset/MasBuildings/test/images/'
    val = os.listdir(source)
    disc = 20
    # 使用多种模型进行测试
    solver = TTAFrame(Instruct_FHNet)
    solver.load('./weights/Instruct_FHNetMasroad_build.th')
    # target = '/data/guoxuejun/yangjialin/CE-Net/submits/log_U_Net/'
    target = './submits/Build/Instruct_FHNetMasroad_build/'

    tic = time()
    if not os.path.exists(target):
        os.mkdir(target)
    # gt_root = '/data/guoxuejun/yangjialin/CE-Net/dataset/ROAD/test/masks'
    gt_root = './dataset/MasBuildings/test/masks1'
    total_m1 = 0
    ...
    for i, name in enumerate(val):
        ...
        text = [10, 1, 2, 8, 4, 5, 6, 7, 11, 0, 0, 0, 0, 0, 0, 0]
        image_path = os.path.join(source, name)
        print(image_path)
        mask = solver.test_one_img_from_path(image_path,text)
        ...
        ground_truth = np.array(cv2.imread(ground_truth_path))[:, :, 0]
